...
export default function link(rootBundle, features) {
    features = features || [ActionTypes, Views, Selectors, EarlyReducers, Reducers, ActionReducers, LateReducers, Sagas];
    const app = {};
...
const ActionTypes = {
    ...
    add: function (app, {actionTypes}) {
        if (actionTypes) {
            Object.assign(app.actionTypes, actionTypes);
        }
    },
...
const Sagas = {
    ...
    finalize: function (app) {
        const effects = app.sagas.map(function (saga) {
            return call(saga);
        });
        app.rootSaga = function* () {
            yield all(effects);
        };
        delete app.sagas;
    }
...
function makeActionReducer({actionTypes, actionReducers}) {
    const map = new Map();
    for (let [key, reducer] of actionReducers.entries()) {
        map.set(actionTypes[key], reducer);
    }
